...
# This checks if any of the path variables are blank, since if they are blank, it could start trying to do things in the / folder, which is not good
	if [[ -z ${FORKED_SRC_FOLDER} || -z ${FORKED_INDI_REPO} ]]
	then
...
then
	echo "No Forked INDI git repo detected.  Please make sure to fork the repo, edit build-env.sh, run setup.sh, and make changes to submit."
	exit
fi
