...
# 	package-StellarBatchSolver.sh - This script uses craft to rebuild and package StellarBatchSolver
...
# Display the Welcome message.
	display "This script will package StellarBatchSolver for Distribution using the requested options."
...
# This sets the version of all the packages as desired if using the -s or -m option.  If these are not specified, it just uses the current settings in craft
	if [ -n "${VERSION}" ]
	then
		display "Setting the version of the StellarBatchSolver package to $VERSION."

		craft --set version="$VERSION" stellarbatchsolver
	fi
	
	craft -i"$VERBOSE" stellarbatchsolver
	craft --package stellarbatchsolver
